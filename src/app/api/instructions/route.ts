...
import { InstructionQuerySchema, InstructionSchema } from "@/lib/api/contracts";
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const query = InstructionQuerySchema.parse(Object.fromEntries(url.searchParams));

    if (isPrototypeMode()) {
      const store = await getPrototypeStore();
      const data = store.listInstructions(query);
      return jsonData(data, prototypeMeta({ total: data.length, limit: data.length || 1, offset: 0 }));
    }

    const response = await liveData("/api/instructions", z.array(InstructionSchema), {
