...
export async function POST(request: Request, context: InstructionRouteContext): Promise<Response> {
  try {
    ...
    if (isPrototypeMode()) {
      ...
      const data = store.decideInstruction(instructionId, "reject", input, idempotencyKey);
      return jsonData(data, prototypeMeta());
    }

    const response = await liveData(`/api/instructions/${instructionId}/reject`, InstructionSchema, {
      method: "POST",
      body: input,
      idempotencyKey,
    });
