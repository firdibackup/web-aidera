...
export async function POST(request: Request, context: InstructionRouteContext): Promise<Response> {
  try {
    ...
    if (isPrototypeMode()) {
      ...
      const data = store.decideInstruction(instructionId, "rollback", input, idempotencyKey);
      return jsonData(data, prototypeMeta());
    }

    const response = await liveData(`/api/instructions/${instructionId}/rollback`, InstructionSchema, {
      method: "POST",
      body: input,
      idempotencyKey,
    });
